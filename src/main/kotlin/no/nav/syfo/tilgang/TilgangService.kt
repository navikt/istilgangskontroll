...
class TilgangService(
    ...
    suspend fun checkTilgangToPersons(
        ...
        val cacheKeysToValues: Map<String, Tilgang?> =
            valkeyStore.getObjects(keys = cacheKeysToPersonident.keys.toList())

        val (cachedEntries, missingEntries) = cacheKeysToValues.entries.partition { it.value != null }
        val cachedTilganger: Map<Personident, Tilgang> = cachedEntries.associate { Personident(it.key) to it.value!! }
